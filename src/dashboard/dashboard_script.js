...
function lireEtMettreAJour() {
  ...
  fetch(fichier)
      ...
      .then(buffer => {
          var classeur = XLSX.read(new Uint8Array(buffer), { type: "array" });

          // Récupérer la première feuille de calcul
          var feuille = classeur.Sheets[classeur.SheetNames[0]];

          // Convertir les données de la feuille en tableau JavaScript
          var donnees = XLSX.utils.sheet_to_json(feuille, { header: 1 });

          // Mettre à jour les styles CSS avec les nouvelles données
          mettreAJourStylesCSS(donnees);
      })
...
function mettreAJourStylesCSS(donnees) {
  ...
  document.querySelector('#bar-example-4 tbody tr:nth-child(2) td').style.setProperty('--size', stockDR14 / stockMaximal);

  // Ajouter temporairement une classe à l'élément pour forcer le navigateur à appliquer les nouveaux styles
  var element = document.querySelector('#bar-example-4');
  element.classList.add('temp-class');
  setTimeout(function() {
      element.classList.remove('temp-class');
  }, 10); // Attendre un bref instant avant de retirer la classe
}
